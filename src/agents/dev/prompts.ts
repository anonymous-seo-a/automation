import { buildAgentMemoryContext, buildSmartAgentContext, AgentRole } from './teamMemory';
...
/** 人格 + セマンティック記憶 + 評価を統合（非同期版・タスク文脈に応じた記憶を注入） */
export async function buildSmartAgentPersonality(agent: AgentRole, taskContext: string): Promise<string> {
  const personality = PERSONALITIES[agent] || '';
  const memoryContext = await buildSmartAgentContext(agent, taskContext);
  const evaluationContext = buildEvaluationContext(agent);
