...
JSONのみ出力。`,
    ...
    if (parsed?.is_evaluation && parsed.target) {
      await saveEvaluation('daiki', parsed.target, parsed.sentiment || 0, parsed.aspect, lastTaskDescription, userMessage);
...
JSONのみ出力: {"sentiment":-1〜+1の整数,"aspect":"品質/効率/自律性","note":"1文の所感"}`,
    ...
    const parsed = safeParseJson(text);
    if (parsed) {
      await saveEvaluation('pm', target, parsed.sentiment || 0, parsed.aspect, taskId, parsed.note);
      await saveAgentMemory(target, 'evaluation', `pm_eval_${Date.now()}`,
...
JSONのみ出力: {"sentiment":-1〜+1の整数,"aspect":"質問力/判断力/正確性/自律性","note":"1文の所感"}`,
    ...
    const parsed = safeParseJson(text);
    if (parsed) {
      await saveEvaluation('pm', target, parsed.sentiment || 0, parsed.aspect, undefined, parsed.note);
    }
  } catch (err) {
...
export async function buildEvaluationContext(target: AgentRole): Promise<string> {
  try {
    const db = getDB();
    const evals = await db.prepare(`
      SELECT * FROM agent_evaluations
      WHERE target = ? AND created_at >= NOW() - INTERVAL '30 days'
...
/** タスクのメトリクスを集計 */
export async function getTaskMetricsSummary(taskId: string): Promise<Record<string, number>> {
  const db = getDB();
